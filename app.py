...
def response_generator(frame, user_input):
    df = frame
    question = f"""
...
st.title("Text-to-Pandas-Code-Generator")
...
if prompt := st.chat_input("What is up?"):
    ...
    generated_code = response_generator(frame, prompt)
    
    # Extract and execute the code block
    start_pos = generated_code.find('```')
    end_pos = generated_code.find('```', start_pos + 3)
    code_block = generated_code[start_pos + 3:end_pos].strip()
    df=frame
    result = code(df,generated_code)
    
    response = f"Echo: {generated_code},"
